...
class customer:
    ...
    def update_customer_list(self, customer_list):
        self.cursor.execute('SELECT CustomerID, CustomerName FROM customer')
        rows = self.cursor.fetchall()
    ...
    def display_customer_table(self):
        self.cursor.execute('SELECT * FROM Customers')
        rows = self.cursor.fetchall()

        if rows:
                print("Customers Table:")
                print(" CustomerID | ProductName  | CustomerName             | Gender | Age | Country | State | County | Email                 | PhoneNumber |  Quantity | Selling_Price | TransactionDate")
                print("-" * 160)
                for row in rows:
    ...
    def add_transaction(self, updated_product_list, updated_customer_list):
        while True:
            try:
                existing_transaction_ids = [row.TransactionID for row in self.cursor.execute("SELECT TransactionID FROM customer")]
                ...
                while product_name in updated_product_list.values():
                    ...
                    if customer_name in updated_customer_list.values():
                        ...
                        # Execute a SQL query to get the maximum CustomerID from the customer table
                        ...
                        # EXECUTE A SQL QUERY TO INSERT CustomerID, CustomerName, Gender, Age, Country, State, County, Email, PhoneNumber INTO TABLE customer
                        self.cursor.execute(
                            "INSERT INTO customer (CustomerID, CustomerName, Gender, Age, Country, State, County, Email, PhoneNumber) "
                            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    ...
                    self.cursor.execute(
                        "INSERT INTO customer (TransactionID, ProductID, CustomerID, ProductName, CustomerName, Gender, Age, Country, State, County, Email, PhoneNumber, Quantity, Selling_Price, TransactionDate) "
                        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    def delete_transaction(self):
        while True:
            try:
                ...
                # Check if the transaction_id exists in the customer table
                if transaction_id not in [row.TransactionID for row in self.cursor.execute("SELECT TransactionID FROM customer")]:
                ...
                transaction_info = self.cursor.execute(
                    f"SELECT * FROM customer WHERE TransactionID = {transaction_id}"
                ).fetchone()

                # Delete the row from customer
                self.cursor.execute(f"DELETE FROM customer WHERE TransactionID = {transaction_id}")
                self.conn.commit()
...
# The following line creates an instance of the customer class and executes the main program loop
if __name__ == "__main__":
    ...
    customer = customer(connection_string)
    
    # Run the program using the instance
    customer.run_program()
