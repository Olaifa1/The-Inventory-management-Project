...
class InventoryManager:
    ...
    def add_new_product(self, updated_product_list):
        while True:
            try:
                new_product_name = input("Enter the new ProductName: ")

                new_product_name = new_product_name.title()  # Convert to Title case


                # Check if new_product_name contains only English alphabets in sentence case
                if not re.match("^[A-Za-z]+( [A-Za-z]+)*$", new_product_name):
                    raise ValueError("Product name must contain only English alphabets.")
        ...
        while True:
            try:
                ...
                while True:
                    ...
                    new_product_name = new_product_name.title() # convert new_product_name to Title case

                    if not re.match("^[A-Za-z]+( [A-Za-z]+)*$", new_product_name):
                        print("New ProductName must contain only English alphabets and have only one space between words.")
                        continue


                    if not new_product_name.strip():
                        print("New ProductName cannot be empty or contain only whitespace characters.")
                        continue
